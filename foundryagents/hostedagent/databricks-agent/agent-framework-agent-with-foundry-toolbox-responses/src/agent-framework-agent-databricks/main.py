...
def is_toolbox_enabled() -> bool:
    """Return whether the Azure Databricks Genie toolbox integration is enabled."""
...
def _install_consent_patch() -> None:
    ...
    Azure Databricks Genie is a remote MCP tool with OAuth identity passthrough:
    on first use the user must sign in to Azure Databricks and approve. Foundry
    reports that as a consent error of type 'mcp'. Older
    `agent_framework_foundry_hosting` builds only recognized some consent types;
    this backport (agent-framework PR #7606) recognizes both 'mcp' and
    ...
    logger.info("Installed Databricks Genie consent recognition patch.")
...
class _ResilientResponsesHostServer(ResponsesHostServer):
    ...
    1. The built-in `_handle_inner_agent` calls `await context.get_history()`
       ...
       to "no prior turns".
    2. Databricks Genie first-run consent is surfaced natively by the SDK as an
...
def main():
    ...
    # Make Databricks Genie first-run consent render as the native sign-in card.
    ...
    # Per-user Azure Databricks identity is supplied server-side by Foundry via the
    # OAuth identity-passthrough connection (AzureDatabricksGenieOBO).
    ...
    agent = Agent(
        client=client,
        instructions=(
            "You are an agent that helps provide answers about Vantia Retail "
            "group's customer churn dataset. Use the Azure Databricks Genie tools "
            "to query the data and answer analytical questions. The Genie tools run "
            "on behalf of the signed-in user (via Azure Databricks OAuth consent), "
            "so only return data that user is permitted to see. Keep answers concise "
            "and include the underlying figures or SQL when it helps the user trust "
            "the result."
        ),
