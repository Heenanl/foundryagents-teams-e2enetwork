...
def is_toolbox_enabled() -> bool:
    """Return whether the SharePoint retrieval toolbox integration is enabled."""
    value = os.getenv("ENABLE_TOOLBOX", "true").strip().lower()
    return value not in {"0", "false", "no", "off"}


def _install_consent_patch() -> None:
    """Surface toolbox OAuth consent errors as the SDK's friendly
    `oauth_consent_request` sign-in card.

    The MCP-OBO gateway sits behind a Foundry OAuth2 identity-passthrough
    connection: on first use the user must approve so Foundry can forward their
    token to the gateway. Foundry reports that as a consent error of type 'mcp'.
    Older `agent_framework_foundry_hosting` builds only recognized some consent
    types; this backport (agent-framework PR #7606) recognizes both 'mcp' and
    'a2a_preview' so the host renders a native sign-in card instead of a raw
    `response.failed`. Harmless no-op on newer SDK builds that already include it.
    """
    try:
        ...
        logger.warning("Could not install consent patch", exc_info=True)
        return
    ...
    logger.info("Installed MCP-OBO gateway consent recognition patch.")
...
class _ResilientResponsesHostServer(ResponsesHostServer):
    ...
    1. The built-in `_handle_inner_agent` calls `await context.get_history()`
       ...
       to "no prior turns".
    2. First-run consent is surfaced natively by the SDK as an
       `oauth_consent_request` card (see `_install_consent_patch`), so no consent
       handling is needed here.
    """

    async def _handle_inner_agent(self, *args, **kwargs):  # type: ignore[override]
        # The SDK's signature for this hook varies across releases, so accept any
        # args and locate the request context (the object exposing get_history)
        # instead of binding to fixed positions.
        context = next(
...
def main():
    ...
    # Make the gateway's first-run consent render as the native sign-in card.
    _install_consent_patch()
    ...
    # The per-user identity used against SharePoint is supplied server-side by
    # Foundry via the OAuth2 identity-passthrough connection (SharePointRetrievalOBO),
    # which forwards the user's token to the MCP-OBO gateway; the gateway does the
    # On-Behalf-Of exchange and calls the Copilot Retrieval API as that user.
    ...
    agent = Agent(
        ...
        instructions=(
            ...
            "Use the SharePoint retrieval tool to fetch relevant extracts, then answer "
            "from those extracts and cite the source document links. The tool runs on "
            "behalf of the signed-in user and is trimmed to what that user is permitted "
            "to see, so never infer or reveal content the tool did not return. If the "
            "tool returns no results, say so plainly rather than guessing."
        ),
